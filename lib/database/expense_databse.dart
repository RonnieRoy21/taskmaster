...
class ExpenseDatabse {
  ...
  Future<Database> getExpenseDatabase() async {
    return openDatabase(
      ...
      onCreate: (db, version) async => db.execute(
        'CREATE TABLE EXPENSES (expense_confirmation_number TEXT PRIMARY KEY ,expense_cost FLOAT NOT NULL,expense_date TEXT NOT NULL,expense_category TEXT NOT NULL)',
      ),
      version: _dbVersion,
    );
  }
  ...
  //delete expense
  Future<void> deleteTask({required String expenseConfirmationNumber}) async {
    final db = await getExpenseDatabase();
    try {
      await db.delete(
        'EXPENSES',
        where: 'expense_confrmation_number = ?',
